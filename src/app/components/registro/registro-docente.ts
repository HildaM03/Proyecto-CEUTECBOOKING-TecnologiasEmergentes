...
export class RegistroDocenteComponent implements OnInit {
  ...
  async registrar() {
    ...
    if (!this.validarFormulario()) {
      ...
      setTimeout(() => {
        if (!this.nombre.trim()) {
          document.getElementById('nombre-docente')?.focus();
        } else if (!this.correo.trim() || !this.validarCorreo()) {
          document.getElementById('correo-docente')?.focus();
        } else if (!this.contrasena.trim() || this.contrasena.length < 8) {
          document.getElementById('contrasena-docente')?.focus();
        }
      }, 100);
    ...
    try {
      ...
      const mensajeError = error.message.includes('correo') ? 
        'Error: El correo ya está registrado o es inválido.' :
  ...
  onKeydown(event: KeyboardEvent, campo: string) {
    // Enter en campos de texto solo activa el submit
    if (event.key === 'Enter' && campo !== 'submit') {
      ...
      if (currentIndex < campos.length - 1) {
        const nextCampo = document.getElementById(`${campos[currentIndex + 1]}-docente`);
        if (nextCampo) {
          nextCampo.focus();
        }
      }
    ...
    if (event.key === 'ArrowUp' || event.key === 'ArrowDown') {
      event.preventDefault();
      const camposIds = ['nombre-docente', 'correo-docente', 'contrasena-docente', 'boton-registro'];
